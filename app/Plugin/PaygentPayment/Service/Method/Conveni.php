 ...
 * コンビニ決済の決済処理を行うクラス.
 */
class Conveni extends AbstractController implements PaymentMethodInterface
{
    /**
     * @var Order
     */
    ...
    public function apply()
    {
        if ($this->Order->getPaymentTotal() > 0) {
            ...
            // リクエストパラメータをセット
            $arrParameter = $this->paygentRequestService->setParameter($this->Order, $config, $this->eccubeConfig['paygent_payment']['payment_type_conveni_num']);
            ...
            if ($arrRet['result'] === $this->eccubeConfig['paygent_payment']['result_success']) {
                // 仮受注完了メールを送信する
                $this->sendConveniOrderMail($this->Order, $arrRet);
            }

            $dispatcher = $this->paygentRequestService->linkPaygentPage($dispatcher, $arrRet, $this->eccubeConfig['paygent_payment']['paygent_conveni_num'], $this->Order);
    ...
    /**
     * コンビニ決済の注文仮完了メールを送信する.
     *
     * @param Order $order
     * @param array $arrRet レスポンスデータ
     */
    public function sendConveniOrderMail($order, $arrRet)
    {
        // お支払期限の整形
        ...
        $message = (new \Swift_Message())
            ...
            ->setBody($this->renderView("@PaygentPayment/default/Shopping/conveni_mail.twig",$params));
