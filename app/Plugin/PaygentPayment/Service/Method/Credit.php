...
class Credit implements PaymentMethodInterface
{
    ...
    public function apply()
    {
        if ($this->Order->getPaymentTotal() > 0) {
            ...
            // リクエストパラメータをセット
            $arrParameter = $this->paygentRequestService->setParameter($this->Order, $config, $this->eccubeConfig['paygent_payment']['payment_type_credit']);
            ...
            $dispatcher = $this->paygentRequestService->linkPaygentPage($dispatcher, $arrRet, $this->eccubeConfig['paygent_payment']['paygent_credit'], $this->Order);
