 ...
 * 携帯キャリア決済の決済処理を行うクラス.
 */
class Credit extends AbstractController implements PaymentMethodInterface
{
    /**
     * @var Order
     */
     ...
     * 注文確認画面遷移時に呼び出される.
     *
     * 携帯キャリア決済のサーバ内でしか行えないような関連チェックを行う.
     *
     */
    ...
    public function apply()
    {
        ...
        if ($this->Order->getPaymentTotal() > 0) {
            //決済画面へ
            logs('paygent_payment')->info("クレジット決済画面へ遷移します");
            $response = new RedirectResponse($this->router->generate('paygent_payment/payment_credit'));
