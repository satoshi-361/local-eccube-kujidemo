 ...
 * コンビニ決済差分通知機能のクラス
 */
class ConveniService extends PaygentBaseService {
    ...
    public function updateOrderStatus($arrRet, $arrParams)
    {
        ...
        if ($order) {
            ...
            $arrIsCancelStatus = [
                ...
                $this->eccubeConfig['paygent_payment']['status_preliminary_cancellation']
            ];
            // リンク型決済 初回("7":決済処理中) かつ キャンセルとなる差分通知以外の時
            if ($order->getOrderStatus()->getId() == OrderStatus::PENDING && !in_array($arrRet['payment_status'], $arrIsCancelStatus)){
            ...
            if (isset($arrParams['paygent_payment_id'])
                && $arrRet['payment_status'] == $this->eccubeConfig['paygent_payment']['status_pre_cleared']
            ) {
    ...
    public function getOrderStatusFromPaygentStatus($paymentStatus, $order)
    {
        switch ($paymentStatus) {
            ...
            case $this->eccubeConfig['paygent_payment']['status_preliminary_pre_detection']: // "43"：速報検知済
                // 受注状態 = "6"：入金済み
                return OrderStatus::PAID;

            case $this->eccubeConfig['paygent_payment']['status_pre_cleared']: // "40"：消込済
                // 速報検知済の通知後に加盟店が対応状況を「発送済み」に更新した場合、消込済の通知で「入金済み」に戻るのを防ぐために消込済通知は「新規受付」、「決済処理中」のみ対象とする．
                if (in_array($order->getOrderStatus()->getId(), [OrderStatus::NEW, OrderStatus::PENDING], true)) {
                    // 受注状態 = "6"：入金済み
                    return OrderStatus::PAID;
                }
                break;
            ...
            case $this->eccubeConfig['paygent_payment']['status_preliminary_cancellation']: // "61"：速報取消済
                // 受注状態 = "3"：キャンセル
    ...
    function isValidStatus($paymentStatus)
    {
        ...
        $arrIsValidStatus = [
            ...
            $this->eccubeConfig['paygent_payment']['status_preliminary_pre_detection'],
            $this->eccubeConfig['paygent_payment']['status_preliminary_cancellation']
        ];
    ...
    function isAlertMail($settlementDivision, $paymentStatus, $order) {
        ...
        if ($settlementDivision == $this->eccubeConfig['paygent_payment']['settlement_id']['link']) {
            // 40 : 消込済
            if ($paymentStatus == $this->eccubeConfig['paygent_payment']['status_pre_cleared']) {
                // 43 : 速報検知済がセットされている場合は除外する
                if ($order->getPaygentPaymentStatus() != $this->eccubeConfig['paygent_payment']['status_preliminary_pre_detection']) {
                    return true;
                }
                // 43 : 速報検知済
            } elseif ($paymentStatus == $this->eccubeConfig['paygent_payment']['status_preliminary_pre_detection']) {
                return true;
            }
        }
