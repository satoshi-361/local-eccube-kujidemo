...
class AdminSoftbankCareerService extends PaygentBaseService {
    ...
    private function setCommitFlags($paygentFlags, $order)
    {
        ...
        if ($paymentMethod == $this->eccubeConfig['paygent_payment']['paygent_career_s']) {
            if ($paygentPaymentStatus == $arrPaygentStatus['status_authority_ok'] or $paygentPaymentStatus == $arrPaygentStatus['status_authority_completed']) {
    ...
    private function setCancelFlags($paygentFlags, $order)
    {
        ...
        if ($paymentMethod == $this->eccubeConfig['paygent_payment']['paygent_career_s']) {
            if (in_array($paygentPaymentStatus, [$arrPaygentStatus['status_authority_ok'], $arrPaygentStatus['status_authority_completed'], $arrPaygentStatus['status_pre_cleared']], true)
                and in_array($paygentPaymentKind, [$arrPaygentKind['paygent_career_commit'], $arrPaygentKind['paygent_career_commit_revice'], null], true)) {
    ...
    private function setChangeFlags($paygentFlags, $order)
    {
        ...
        if ($paymentMethod == $this->eccubeConfig['paygent_payment']['paygent_career_s']) {
            if ($paygentPaymentKind == null and in_array($paygentPaymentStatus, [$arrPaygentStatus['status_authority_ok'], $arrPaygentStatus['status_authority_completed']], true)) {
