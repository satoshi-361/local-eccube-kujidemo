...
class AdminDocomoCareerService extends PaygentBaseService {
    ...
    private function setCommitFlags($paygentFlags, $order)
    {
        ...
        if ($paymentMethod == $this->eccubeConfig['paygent_payment']['paygent_career_d']) {
    ...
    private function setCancelFlags($paygentFlags, $order)
    {
        ...
        if ($paymentMethod == $this->eccubeConfig['paygent_payment']['paygent_career_d']) {
            if (($paygentPaymentKind == null and in_array($paygentPaymentStatus, [$arrPaygentStatus['status_authority_ok'], $arrPaygentStatus['status_authority_completed']], true))
                or ($paygentPaymentStatus == $arrPaygentStatus['status_complete_cleared'] and in_array($paygentPaymentKind, [$arrPaygentKind['paygent_career_commit'], $arrPaygentKind['paygent_career_commit_revice']], true))) {
