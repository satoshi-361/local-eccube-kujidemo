...
class HttpsRequestSender {
	...
	/** デバッグログマスク対象項目 */
	var $MASK_COLUMNS = ["card_number", "card_conf_number"];
	...
	public function __construct($url, EccubeConfig $eccubeConfig)
	{
		$this->HttpsRequestSender($url);
...
//			$this->outputDebugLog("param: " . $key . " = \"" . $value . "\"");
			...
			if ($debugLogFlg and in_array($key, $this->MASK_COLUMNS) and !StringUtil::isEmpty($value)) {
				...
				$value = str_repeat($this->eccubeConfig['paygent_payment']['httpsrequestsender__mask_string'], strlen($value) - 1) . substr($value, -1);
			}
	...
	function outputDebugLog($msg) {
		...
		$inst = PaygentB2BModuleLogger::getInstance($this->eccubeConfig);
		if (is_object($inst)) {
	...
	function send($formData, $debugFlg) {
		...
		$header = [];
		$header[] = $this->eccubeConfig['paygent_payment']['httpsrequestsender__content_type'];
		$header[] = $this->eccubeConfig['paygent_payment']['httpsrequestsender__http_encoding'];
		$header[] = $this->eccubeConfig['paygent_payment']['httpsrequestsender__content_length'] . ": "
			. (StringUtil::isEmpty($query)? "0" : strlen($query));
		$header[] = $this->eccubeConfig['paygent_payment']['httpsrequestsender__user_agent'] . ": " . "curl_php";
	...
	function procError() {
		...
		if ($errorNo <= $this->eccubeConfig['paygent_payment']['httpsrequestsender__curle_couldnt_connect']) { // 7
			// 接続問題
			$retCode = $this->eccubeConfig['paygent_payment']['paygentb2bmoduleconnectexception__ks_connect_error'];
			$this->outputDebugLog($errorMsg);
		} else if ($errorNo == $this->eccubeConfig['paygent_payment']['httpsrequestsender__curle_couldnt_connect']) { // 7
			// 接続問題
			$retCode = $this->eccubeConfig['paygent_payment']['paygentb2bmoduleconnectexception__ks_connect_error'];
			$this->outputDebugLog($errorMsg);
		} else if ($errorNo == $this->eccubeConfig['paygent_payment']['httpsrequestsender__curle_ssl_certproblem']) {
			// 認証問題
			$retCode = $this->eccubeConfig['paygent_payment']['paygentb2bmoduleconnectexception__certificate_error'];
			$this->outputDebugLog($errorMsg);
		} else if ($errorNo == $this->eccubeConfig['paygent_payment']['httpsrequestsender__curle_ssl_cacert']) {
			// 認証問題
			$retCode = $this->eccubeConfig['paygent_payment']['paygentb2bmoduleconnectexception__certificate_error'];
			$this->outputDebugLog($errorMsg);
		} else if ($errorNo == $this->eccubeConfig['paygent_payment']['httpsrequestsender__curle_ssl_cacert_badfile']) {	// CURLE_SSL_CACERT_BADFILE
			// 認証問題
			$retCode = $this->eccubeConfig['paygent_payment']['paygentb2bmoduleconnectexception__certificate_error'];
			$this->outputDebugLog($errorMsg);
		} else if ($errorNo == $this->eccubeConfig['paygent_payment']['httpsrequestsender__curle_http_returned_error']) {
		...
		// 証明書ファイルの状態チェック
		foreach ([$this->clientCertificatePath, $this->caCertificatePath] as $path) {
			if (!file_exists($path)) {
	...
	function parseResponse($data) {
		...
		// レスポンス受信
		$line = null;
		$retCode = $this->eccubeConfig['paygent_payment']['httpsrequestsender__http_status_init_value'];
		$bHeaderOver = false;
		$resBodyStart = 0;
		$lines = preg_split($this->eccubeConfig['paygent_payment']['httpsrequestsender__crlf'], $data);
		// ヘッダまでを読み込む
		foreach($lines as $i => $line) {
			...
			$resBodyStart += strlen($line) + strlen($this->eccubeConfig['paygent_payment']['httpsrequestsender__crlf']);

			if ($retCode === $this->eccubeConfig['paygent_payment']['httpsrequestsender__http_status_init_value']) {
				// ステータスの解析
	...
	function parseStatusLine($line) {
		...
		if (strpos($statusLine[0], $this->eccubeConfig['paygent_payment']['httpsrequestsender__http'] . "/") != 0
				|| !StringUtil::isNumericLength($statusLine[1], $this->eccubeConfig['paygent_payment']['httpsrequestsender__regexpstatus_len'])) {

			// 不正なステータスコードを受け取った
			return $this->eccubeConfig['paygent_payment']['paygentb2bmoduleconnectexception__ks_connect_error'];
		}

		if (!(($this->eccubeConfig['paygent_payment']['httpsrequestsender__http_success'] <= $this->statusCode)
			&& ($this->statusCode <= $this->eccubeConfig['paygent_payment']['httpsrequestsender__http_partial_content']))) {
