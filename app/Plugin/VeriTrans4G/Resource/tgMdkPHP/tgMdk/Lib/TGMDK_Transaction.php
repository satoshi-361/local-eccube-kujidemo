...
class TGMDK_Transaction {
    ...
    private function createSendUrl($requestDto, $serviceType, $serviceCommand, $conf_array) {
        ...
        if (in_array($serviceType, $serviceTypeArray)) {
            ...
            if (isset($conf_array[TGMDK_Config::PAYNOWID_API_CONTEXT])
                && !empty($conf_array[TGMDK_Config::PAYNOWID_API_CONTEXT])) {
                // ペイナウAPIコンテキストが設定されている場合はserviceNameを上書き
                $serviceName = $conf_array[TGMDK_Config::PAYNOWID_API_CONTEXT];
            }
